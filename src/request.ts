...
export class Request<Return_Type = any, Error_Handler = never> {
     ...
     * @param callback A function to call when the request is finished
     */
    private sendWithAxios(callback?: () => void) {
        ...
        axios.request({
            ...
            data: this.json || this.form || undefined,
            onUploadProgress: progressEvent => {
                this._onUploadProgress && this._onUploadProgress(progressEvent.loaded || 0, progressEvent.total || 0);
            },
        }).then(response => {
            if (!this.aborted) {
                callback && callback();
